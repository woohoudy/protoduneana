...
pdhd::MichelTimingDataDeconHD::MichelTimingDataDeconHD(
...
void pdhd::MichelTimingDataDeconHD::analyze(art::Event const &e) {
  ...
  //  auto wfListHandle = e.getHandle< std::vector<raw::OpDetWaveform>
  //  >("pdhddaphne:daq");
  auto wfListHandle =
      e.getHandle<std::vector<recob::OpWaveform>>("opdec::RecoDeconv");
  auto wf2ListHandle =
      e.getHandle<std::vector<raw::RDTimeStamp>>("timingrawdecoder:daq");
  pdchannel.clear();
  pdt0.clear();
  ...
  for (const auto &wf : *wfListHandle) { // Use reference instead of copying
    if (wf.Channel() < 120 &&
        wf.Signal().size() <= 1024) { // repalce ChannelNumber by Channel
      pdchannel.push_back(wf.Channel());
      pdt0.push_back((wf.TimeStamp() - t0) * 16 *
                     1e-3); // 16ns/tick, 1e-3: in unit of us---
    }
  ...
  //  std::cout<<"\n\nOpch<120: Num of Long(>1024) wvf:
  //  "<<nWFLong<<"\n"<<std::endl; std::cout<<"Opch<120: Num of Short(<1024)
  //  wvf: "<<nWFShort<<"\n"<<std::endl; std::cout<<"Opch<120: Num of
  //  Normal(1024) & Short wvf: "<<nWF<<"\n"<<std::endl;

  //  std::cout<<"\nAll OpChs: Num of All([0, inf)) wvf:
  //  "<<nWFTotal<<"\n\n"<<std::endl;
