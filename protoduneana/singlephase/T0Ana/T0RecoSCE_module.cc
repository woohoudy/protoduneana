...
void T0RecoSCE::analyze(art::Event const & evt){
    ...
    if(fUseOpHits) op_hit_h = evt.getHandle<std::vector<recob::OpHit> >(fOpHitProducer);
        ...
        // Determine if the track crosses the cathode
            ...
            auto const [start_axis, start_sign] = geom->TPC(wireID).DriftAxisWithSign();
            geo::DriftAxis driftDir_end{};

            for (size_t ii = 1; ii < hit_v.size(); ii++) {
                const geo::WireID wireID2 = hit_v.at(ii)->WireID();
                driftDir_end = geom->TPC(wireID2).DriftAxisWithSign();
                auto const [axis, sign] = driftDir_end;
            ...
            auto const [end_axis, end_sign] = driftDir_end;
        ...
        if (fUseMC){
                ...
                length = track->Length();
                        ...
                        if(cathode_crossing_track&&cathode_rc_time>-fReadoutWindow) {
                          x_shift = cathode_rc_time*to_int(start_sign)*fDriftVelocity;
                        }
                        ...
                        rc_xs_corr = rc_xs - x_shift + to_int(start_sign)*anode_rc_time*fDriftVelocity;
                        rc_xe_corr = rc_xe + x_shift + to_int(end_sign)*anode_rc_time*fDriftVelocity;
                        ...
                        if(!fUseOpHits) {
                                ...
                                matched_flash_pe = flash_ptr->TotalPE();
                                        ...
                                        double max_pe = 0;
                                        unsigned int pd_ch;
                                        for(pd_ch = 0; pd_ch <= wireReadout.MaxOpChannel(); pd_ch++) {
